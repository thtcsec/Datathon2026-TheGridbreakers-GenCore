...
Output:
  output/submission_raw_stable_neural_blend_w73_w56_residual.csv
"""
...
TARGET_COGS_RATIO = 0.8440
REV_NEURAL_WEIGHT = 0.73
COGS_NEURAL_WEIGHT = 0.56
...
def train_residual_refiner(X_train, residual, X_test, target):
    """
    Residual refiner: học phần sai số còn lại log(actual) - log(anchor).
    Không dùng làm model chính, chỉ correction nhẹ để tránh overfit.
    """
    residual = np.asarray(residual, float)
    residual = np.clip(residual, np.percentile(residual, 2), np.percentile(residual, 98))
    ...
    preds = []
    weights = np.array([0.18, 0.47, 0.35])
    for name, model in models:
        print(f"    residual-refiner {target}: training {name}")
        model.fit(X_train, residual)
        preds.append(model.predict(X_test))

    pred = np.average(np.vstack(preds), axis=0, weights=weights)
    pred = np.clip(pred, -0.22, 0.22)
    pred = pred - np.mean(pred)  # chỉ sửa shape, không đổi level
    return pred


def apply_residual_refinement(sales, sub, promos, traffic, inventory,
                              oof_base, revenue, cogs):
    """
    Dùng OOF anchor từ stable model để học residual trên train,
    rồi sửa rất nhẹ prediction test. Đây là raw-only, không đọc submission cũ.
    """
    print("[5/5] Applying conservative residual refinement...")

    # Train anchor tương ứng với stable anchor, dùng OOF để giảm leakage.
    train_anchor_rev, train_anchor_cogs = calibrate_anchor(
        sales["Date"],
        np.maximum(oof_base["Revenue"], 1.0),
        np.maximum(oof_base["COGS"], 1.0),
    )

    X_train = make_neural_features(sales["Date"], promos, traffic, inventory)
    X_test = make_neural_features(sub["Date"], promos, traffic, inventory)

    rev_resid = np.log1p(sales["Revenue"].to_numpy(float)) - np.log1p(np.maximum(train_anchor_rev, 1.0))
    cogs_resid = np.log1p(sales["COGS"].to_numpy(float)) - np.log1p(np.maximum(train_anchor_cogs, 1.0))

    rev_corr = train_residual_refiner(X_train, rev_resid, X_test, "Revenue")
    cogs_corr = train_residual_refiner(X_train, cogs_resid, X_test, "COGS")

    # Shrink nhỏ: nếu residual model đúng sẽ giảm, nếu sai thì không phá quá nhiều.
    REV_RESIDUAL_SHRINK = 0.10
    COGS_RESIDUAL_SHRINK = 0.08

    old_rev_mean = np.mean(revenue)
    old_cogs_mean = np.mean(cogs)

    revenue2 = revenue * np.exp(REV_RESIDUAL_SHRINK * rev_corr)
    cogs2 = cogs * np.exp(COGS_RESIDUAL_SHRINK * cogs_corr)

    # Preserve level của best w73/w56; chỉ đổi daily shape.
    revenue2 *= old_rev_mean / max(np.mean(revenue2), 1.0)
    cogs2 *= old_cogs_mean / max(np.mean(cogs2), 1.0)

    # Blend residual-refined với bản best để giảm rủi ro.
    revenue_final = 0.72 * revenue + 0.28 * revenue2
    cogs_final = 0.72 * cogs + 0.28 * cogs2

    return np.clip(revenue_final, 0, None), np.clip(cogs_final, 0, revenue_final * 1.45)


def main():
    print("=" * 80)
    print("RAW-ONLY STABLE + OPTIMAL SPLIT + RESIDUAL REFINER")
    print("=" * 80)
    ...
    revenue, cogs = apply_residual_refinement(sales, sub, promos, traffic, inventory, oof_base, revenue, cogs)
    ...
    out_path = os.path.join(OUT_DIR, "submission_raw_stable_neural_blend_w73_w56_residual.csv")
